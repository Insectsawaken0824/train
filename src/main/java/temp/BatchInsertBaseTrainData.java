...
//        insertTrainDataToMysql("D:\\train\\Data.txt");
        insertCityDataToMysql("F:\\train\\Data\\city.txt");
        pst.close();
        conn.close();
    }

    public static void insertCityDataToMysql(String path) throws Exception {
        List<String> list = FileUtils.readLines(new File(path));
        HashMap<String, HashMap<String, List<String>>> map = new HashMap<String, HashMap<String, List<String>>>();
        for (String line : list) {
            String[] split = line.split("\t");
            String province = split[0].replace("市","").replace("省","").replace("自治区","");
            String city = split[1];
